...
A: There are several major reasons for choosing `.json` files to be used instead of `.html` files for the application. Firstly, parsing data is arguaby more difficult and tedious in `.html` format, while `.json` provides data in a more structured and flexible way which is easier for programmers to work with, especially when creating web applications for data. Secondly, `.json` files typically can store a larger amount of data compared to `.html` files. Uploading a 100MB `.json` file to the app is much more convenient than several `.html` files, from both the programmers and the user's perspective. Lastly, due to how Telegram messages are displayed in `.html` format, there is a high likelihood of information loss during data processin if one were to simply treat the `.html` file as a webpage and use a web scraping library to scrape data from the file.
